...
class Track:
    ...
    def update_geometry(self, det: FaceDetection, frame_idx: int) -> None:
        raw_cx = (det.x1 + det.x2) / 2.0
        raw_cy = (det.y1 + det.y2) / 2.0
        prev_cx, prev_cy = self.center
        a = config.FACE_CENTER_SMOOTHING
        if self._smooth_cx is None:
            self._smooth_cx, self._smooth_cy = raw_cx, raw_cy
        else:
            self._smooth_cx = a * raw_cx + (1.0 - a) * self._smooth_cx
            self._smooth_cy = a * raw_cy + (1.0 - a) * self._smooth_cy
        new_center = (self._smooth_cx, self._smooth_cy)
        self.velocity = (new_center[0] - prev_cx, new_center[1] - prev_cy)
